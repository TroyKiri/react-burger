...
function IngredientDetails() {
  ...
  const { currentIngredient } = useSelector(store => store.ingredientReducer);

  return (
    <div className={ingredientDetailsStyles.container}>
      <img className={`${ingredientDetailsStyles.image} mb-4`} src={currentIngredient.image_large} alt={currentIngredient.name} />
      <h2 className='text text_type_main-medium mb-8' >{currentIngredient.name}</h2>
